...
for i in experiments:
    ...
    source = ",".join(target_names)

    for g in gene_names:
        target=g
        max_number_of_paths = 1
        url = directed_path_query_url + '?source=' + source + '&target=' + target + '&pathnum=' + str(max_number_of_path)

        response=requests.post(url, files={"network_cx": f})

        heat_response_dict[i]=len(list(response.json()['forward']))

fh=open('path_length_response.txt','w')
